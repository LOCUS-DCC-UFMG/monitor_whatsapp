...
def get_audios_by_date(conn, obtained_at, offset,  end_date):
    ...
    cursor.execute(sql, data)
    response = cursor.fetchall()
    response =  list(map(lambda x: {'name': "Áudio", 'url': 'http://www.monitor-de-whatsapp.dcc.ufmg.br/data/audios/' + str(x[0]), 'audioid': x[0], 'rankingDay': x[1],  'shareNumber': x[2], 'shareNumberUsers': x[3], 'shareNumberGroups': x[4], 'obtained_at': x[5].strftime('%m/%d/%Y'), 'shared_groups': x[6], 'shared_users': x[7], 'duration': x[8], 'checksum': x[9]}, response))
