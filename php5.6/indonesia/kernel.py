...
def connect_app_database():
    conn_string = "host=localhost dbname=whatsapp_indonesia user=whomakestrends password=678hjba!@1ABCUyvs8"
...
def get_images_by_date(conn, obtained_at, offset,  end_date):
    ...
    sql = "SELECT imageID, rankingday, sharenumber, sharenumberusers, sharenumbergroups, obtained_at, shared_groups, shared_users, checksum, phash, nsfw_score FROM {table_name} WHERE ".format(table_name=whatsapp_image_table) +      date_condition + " ORDER BY sharenumber DESC, sharenumberusers DESC, sharenumbergroups DESC LIMIT {max_images};".format(max_images=max_images)
    data = list()
    cursor.execute(sql, data)
    response = cursor.fetchall()
    response =  list(map(lambda x: {'imageid': x[0], 'rankingDay': x[1],  'shareNumber': x[2], 'shareNumberUsers': x[3], 'shareNumberGroups': x[4], 'obtained_at': x[5].strftime('%m/%d/%Y'), 'shared_groups': x[6], 'shared_users': x[7], 'checksum': x[8],'phash': x[9] ,'nsfw_score': x[10]}, response))
...
def get_audios_by_date(conn, obtained_at, offset,  end_date):
    ...
    cursor = conn.cursor()
    date_condition = check_end_date_condition(obtained_at, end_date)
    sql = "SELECT audioID, rankingday, sharenumber, sharenumberusers, sharenumbergroups, obtained_at, shared_groups, shared_users, duration, checksum FROM {table_name} WHERE ".format(table_name=whatsapp_audio_table) +           date_condition + " ORDER BY sharenumber DESC, sharenumberusers DESC, sharenumbergroups DESC LIMIT {max_audios};".format(max_audios=max_audios)
    data = (obtained_at, )
    cursor.execute(sql, data)
    response = cursor.fetchall()
    response =  list(map(lambda x: {'name': "Áudio", 'url': 'http://www.monitor-de-whatsapp.dcc.ufmg.br/indonesia/data/audios/' + str(x[0]), 'audioid': x[0], 'rankingDay': x[1],  'shareNumber': x[2], 'shareNumberUsers': x[3], 'shareNumberGroups': x[4], 'obtained_at': x[5].strftime('%m/%d/%Y'), 'shared_groups': x[6], 'shared_users': x[7], 'duration': x[8], 'checksum': x[9]}, response))
...
def get_videos_by_date(conn, obtained_at, offset, end_date):
    cursor = conn.cursor()
    date_condition = check_end_date_condition( obtained_at, end_date)
    sql = "SELECT videoid, rankingday, sharenumber, sharenumberusers, sharenumbergroups, obtained_at, shared_groups, shared_users, checksum, nsfw_score  FROM {table_name} WHERE ".format(table_name=whatsapp_video_table) +    date_condition + "ORDER BY sharenumber DESC, sharenumberusers DESC LIMIT {max_videos};".format(max_videos=max_videos)
    data = list()
    cursor.execute(sql, data)
    response = cursor.fetchall()
    response = list(map(lambda x: {'videoid': x[0], 'rankingDay': x[1],  'shareNumber': x[2], 'shareNumberUsers': x[3], 'shareNumberGroups': x[4], 'obtained_at': x[5].strftime('%m/%d/%Y'), 'shared_groups': x[6], 'shared_users': x[7], 'checksum': x[8], 'nsfw_score': x[9]}, response))
...
def main():
    arg = sys.argv[1]
    p = '{"op": 1010, "obtained_at": "2019-01-25", "end_date": "2019-01-30", "offset": 0}'
    ...
    if op == 1010:
        ...
        results = {'images': images, 'audios': audios, 'links':links, 'messages':messages, 'videos':videos}
